...
class UpdateRooms(Frame):
    def __init__(self, parent, controller=None, *args, **kwargs):
        ...
        self.entry_2.place(x=455.0, y=170.0, width=269.0, height=22.0)
        ...
        self.canvas.create_text(
            ...
            253.0,
            anchor="nw",
            text="Tình trạng sử dụng:(T)rống/(D)ùng",
            fill="#5E95FF",
            font=("Montserrat Bold", 14 * -1),
        )
        ...
        self.entry_3 = Entry(
            self,
            font=("Montserrat Bold", 18 * -1),
            textvariable=self.data["occupation"],
            foreground="#777777",
            bd=0,
            bg="#EFEFEF",
            highlightthickness=0,
    ...
    def initialize(self):
        ...
        # Lọc lấy thông tin phòng đang chọn
        ...
        if self.selected_rooms_data:
            ...
            self.data["type"].set(loaiphong.get(self.selected_rooms_data[2]))
            self.data["occupation"].set("Còn trống" if self.selected_rooms_data[4] == 1 else "Đang sử dụng")
    ...
    def handle_update(self):
        ...
        result = db_controller.update_rooms(
            ...
            room_type=loaiphong.get(self.data["type"].get().lower()),
            occupation="1" if self.data["occupation"].get().lower() == "t" else "0",
        )

        if result:
            messagebox.showinfo(None, "Cập nhật thông tin thành công!")
