...
class ViewRooms(Frame):
    def __init__(self, parent, controller=None, *args, **kwargs):
        ...
        self.image_1 = self.canvas.create_image(666.0, 59.0, image=self.image_image_1)

        self.entry_image_1 = PhotoImage(file=relative_to_assets("entry_1.png"))
        self.entry_bg_1 = self.canvas.create_image(680.5, 60.0, image=self.entry_image_1)
        self.entry_1 = Entry(
            self,
            bd=0,
            bg="#EFEFEF",
        ...
        self.image_2 = self.canvas.create_image(617.0, 60.0, image=self.image_image_2)
        ...
        self.columns = {
            "Mã phòng": ["Mã phòng", 50],
            "Số phòng": ["Số phòng", 80],
            "Loại": ["Loại", 100],
            "Giá": ["Giá", 110],
            "Tình trạng": ["Tình trạng", 150],
            "Ngày nhập": ["Ngày nhập", 200],
        }
    ...
    def handle_refresh(self):
        ...
        for row in self.room_data:
            row = list(row) 
            # --- Xử lý cột tình trạng (boolean -> text) ---
            if row[4] in (1, True):
                row[4] = "Còn trống"
            else:
                row[4] = "Đang sử dụng"
            ...
            if isinstance(row[3], (int, float)):
                row[3] = f"{row[3]:,}".replace(",", ".")
            
            # --- Xử lý hiện thị cột Loại phòng ---
            loaiphong = {
                "don": "Đơn",
                "doi": "Đôi",
                "vip": "VIP"
            }
            row[2] = loaiphong.get(row[2])
    ...
    def handle_delete(self):
        if db_controller.delete_room(self.parent.selected_rid):
            messagebox.showinfo(None,"Xoá thành công!")
        else:
            messagebox.showerror("Lỗi","Không thể xoá phòng!")
